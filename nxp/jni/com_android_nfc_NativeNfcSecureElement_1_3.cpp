...
#define WTX_MULTUPLIER_TO  257U
#define JNI_WTX_TIMER_MS  (4 * ((uint16_t)NXP_NFC_WTX_TIMEOUT >> 8) * WTX_MULTUPLIER_TO )
namespace android {

extern void jniTimerCreateWTX(void);
extern void jniTimerStartWTX(uint32_t RegTimeCnt);
extern void jniTimerDeleteWTX(void);
extern void jniTimerStopWTX(void);
                                               ...
                                               NFCSTATUS        status)
{
   ...
   if (status == NFCSTATUS_SUCCESS)
   {
       ...
       TRACE("SE Handle: %u", hSecureElement);
   }
   else
   {
   ...
   jobject o,jint handle, jbyteArray data)
{
   ...
   transceive_info.timeout = NXP_NFC_WTX_TIMEOUT | 0x8000;
   REENTRANCE_LOCK();
   #if ( ES_HW_VER < 33 ) || ( NXP_NFC_EXT_APDU_SUPPORT == 0x00 )
   Hci_timeout=phLibNfc_GetHciTimeout();
   phLibNfc_SetHciTimeout(NXP_NFC_HCI_TIMER_DISABLE);
   #endif
   status = phLibNfc_RemoteDev_Transceive(handle, &transceive_info,
                   com_android_nfc_jni_transceive_callback, (void *)&cb_data);
   #if ( ES_HW_VER < 33 ) || ( NXP_NFC_EXT_APDU_SUPPORT == 0x00 )
   phLibNfc_SetHciTimeout(Hci_timeout);
   #endif
   REENTRANCE_UNLOCK();
   if(NFCSTATUS_PENDING == status)
   {
      jniTimerStartWTX(JNI_WTX_TIMER_MS);
   }
   else
   {
