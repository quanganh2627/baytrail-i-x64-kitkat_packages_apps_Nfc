...
/* TODO temporary */
static phLibNfc_sEmulationCfg_t pConfigInfo;

uint8_t device_connected_flag;
//#ifdef NXP_NFC_P2P_PRIO_SW
static uint8_t typeA_flag = TRUE;
static uint8_t typeB_flag = TRUE;
//#endif
...
static jmethodID cached_NfcManager_notifyCEFHActivated;
static jmethodID cached_NfcManager_notifyCEFHDeActivated;
...
#ifdef REMOVE_THIS
static uint16_t FelicaTimeout;
static uint16_t IsoXchgTimeout;
static uint16_t MifareRawTimeout;
#else
static uint32_t FelicaTimeout;
static uint32_t IsoXchgTimeout;
static uint32_t MifareRawTimeout;
#endif
...
static void nfc_jni_se_set_mode_callback(void *context, phLibNfc_Handle handle,
                                         NFCSTATUS status);
...
static void nfc_jni_start_discovery_locked(struct nfc_jni_native_data *nat,
                                           bool resume);
static void nfc_jni_Discovery_notification_callback(
        void *pContext, phLibNfc_RemoteDevList_t *psRemoteDevList,
        uint8_t uNofRemoteDev, NFCSTATUS status);
static void nfc_jni_cefh_notification_callback(
        void *pContext, phLibNfc_RemoteDevList_t *psRemoteDevList,
        uint8_t uNofRemoteDev, NFCSTATUS status);
static void nfc_jni_transaction_callback(void *context,
                                         phLibNfc_eSE_EvtType_t evt_type,
                                         phLibNfc_Handle handle,
                                         phLibNfc_uSeEvtInfo_t *evt_info,
                                         NFCSTATUS status);
...
static void kill_client(nfc_jni_native_data *nat)
{
    ...
    phDal4Nfc_msgsnd(gDrvCfg.nClientId, (struct msgbuf *) &wrapper,
            sizeof(phLibNfc_Message_t), 0);
}

static void nfc_jni_ioctl_callback(void *pContext, phNfc_sData_t *pOutput,
                                   NFCSTATUS status)
{
...
static void nfc_jni_set_multise_routing_table_callback(void *pContext,
                                                       NFCSTATUS status)
{
    struct nfc_jni_callback_data * pCallbackData =
            (struct nfc_jni_callback_data *) pContext;
    ALOGE("nfc_jni_set_multise_routing_table_callback %02x", status);

    /* Report the callback status and wake up the caller */
    pCallbackData->status = status;
    sem_post(&pCallbackData->sem);
}
static int nfc_jni_download_locked(struct nfc_jni_native_data *nat,
                                   uint8_t update)
{
    ...
    {
        TRACE("--> %s goto clean_and_return",__FUNCTION__);
    ...
    {
        //deinit
        TRACE("--> %s update",__FUNCTION__);
        TRACE("phLibNfc_Mgt_DeInitialize() (download)");
        REENTRANCE_LOCK();
        status = phLibNfc_Mgt_DeInitialize(gHWRef,
                nfc_jni_deinit_download_callback, (void *) &cb_data);
        REENTRANCE_UNLOCK();
        if (status != NFCSTATUS_PENDING)
        {
            ALOGE("phLibNfc_Mgt_DeInitialize() (download) returned 0x%04x[%s]",
    ...
    REENTRANCE_LOCK();
    status = phLibNfc_Mgt_Initialize(gHWRef, nfc_jni_init_callback,
            (void *) &cb_data);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_Mgt_Initialize() (download) returned 0x%04x[%s]",
                status, nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_Mgt_Initialize() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
    ...
    /* ====== CAPABILITIES ======= */REENTRANCE_LOCK();
    status = phLibNfc_Mgt_GetstackCapabilities(&caps, (void*) nat);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_SUCCESS)
    {
        ALOGW("phLibNfc_Mgt_GetstackCapabilities returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
    } else
    {
        ALOGD(
                "NFC capabilities: HAL = %x, FW = %x, HW = %x, Model = %x, HCI = %x, Full_FW = %d, Rev = %d, FW Update Info = %d",
...
static int nfc_jni_configure_driver(struct nfc_jni_native_data *nat)
{
    ...
    {
        ALOGW("phLibNfc_Mgt_ConfigureDriver() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
    } else if (status != NFCSTATUS_SUCCESS)
    {
        ALOGE("phLibNfc_Mgt_ConfigureDriver() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_Mgt_ConfigureDriver() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
    ...
    clean_and_return: return result;
}
...
static int nfc_jni_unconfigure_driver(struct nfc_jni_native_data *nat)
{
    ...
    {
        ALOGE(
                "phLibNfc_Mgt_UnConfigureDriver() returned error 0x%04x[%s] -- this should never happen",
                status, nfc_jni_get_status_name(status));
    } else
    {
        ALOGD("phLibNfc_Mgt_UnConfigureDriver() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        result = TRUE;
    }
...
static int nfc_jni_initialize(struct nfc_jni_native_data *nat)
{
    ...
    uint8_t i, No_SE = PHLIBNFC_MAXNO_OF_SE, SmartMX_index = 0,
            SmartMX_detected = 0;
    ...
    int ret = 0;
    uint8_t UiccSetModeOff[4] =
        { 0x00, 0x9F, 0x17, 0x00 };
    uint8_t uiccSessionResp[8];
    uint8_t uiccSessionExResp[8] =
    ...
    /*if (pn544_dev->num_eeprom_settings == 0
            || pn544_dev->eeprom_settings == NULL)
    {
    ...
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_Mgt_Initialize() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        update = FALSE;
        goto force_download;
    }
    ...
    if (cb_data.status != NFCSTATUS_SUCCESS)
    {
        TRACE("cb_data.status = 0x%04x",cb_data.status );
        update = FALSE;
        goto force_download;
    }
...
//FA force download
//   caps.psDevCapabilities.firmware_update_info=1;
//end FA
    /* ====== FIRMWARE VERSION ======= */
    if (caps.psDevCapabilities.firmware_update_info)
    {
        force_download: for (i = 0; i < 3; i++)
        {
    ...
    for (i = 0; i < pn544_dev->num_eeprom_settings; i++)
    {
        ...
        snprintf(eeprom_property, sizeof(eeprom_property),
                "debug.nfc.eeprom.%02X%02X", eeprom_base[1], eeprom_base[2]);
        TRACE(">> Checking property: %s", eeprom_property);
        if (property_get(eeprom_property, eeprom_value, "") == 2)
        {
            int eeprom_value_num = (int) strtol(eeprom_value, (char**) NULL,
                    16);
        ...
        TRACE(
                ">> Addr: 0x%02X%02X set to: 0x%02X", eeprom_base[1], eeprom_base[2], eeprom_base[3]);
        gInputParam.buffer = eeprom_base;
        gInputParam.length = 0x04;
        gOutputParam.buffer = resp;

        REENTRANCE_LOCK();
        status = phLibNfc_Mgt_IoCtl(gHWRef, NFC_MEM_WRITE, &gInputParam,
                &gOutputParam, nfc_jni_ioctl_callback, (void *) &cb_data);
        REENTRANCE_UNLOCK();
        if (status != NFCSTATUS_PENDING)
        {
            ALOGE("phLibNfc_Mgt_IoCtl() returned 0x%04x[%s]", status,
                    nfc_jni_get_status_name(status));
            goto clean_and_return;
        }
        /* Wait for callback response */
    ...
    /* Cache the Secure Element List and Count in the nat */
    nat->No_SE = No_SE;

    memcpy(nat->SE_List, SE_List, sizeof(SE_List));
    ...
    for (i = 0; i < No_SE; i++)
    {
        ...
        {
            ALOGD("phLibNfc_SE_GetSecureElementList(): SMX detected, handle=%p",
                    (void*) SE_List[i].hSecureElement);
        } else if (SE_List[i].eSE_Type == phLibNfc_SE_Type_UICC)
        {
            ALOGD(
                    "phLibNfc_SE_GetSecureElementList(): UICC detected, handle=%p",
                    (void*) SE_List[i].hSecureElement);
            TRACE("Checking for UICC session FLAG");
            ...
            {
                usleep(200000);
                memset(uiccSessionResp, 0x00, sizeof(uiccSessionResp));
                com_android_nfc_NfcManager_getUICCSessionData(uiccSessionResp);

                if (0
                        == memcmp(uiccSessionResp, uiccSessionExResp,
                                sizeof(uiccSessionExResp)))
                {
                    TRACE("Waiting for UICC Session flag change");
                    ALOGD("Waiting for UICC Session flag change");
                    continue;
                } else
                {
                    TRACE("UICC Session flag changed");
                    ALOGD("UICC Session flag changed");
                    break;
                }
            } while ((icount++) < maxretry);
    ...
    status = phLibNfc_Mgt_SetLlcp_ConfigParams(&LlcpConfigInfo,
            nfc_jni_llcpcfg_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_PENDING)
    {
    ...
    TRACE(
            "phLibNfc_Mgt_SetLlcp_ConfigParams returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
    ...
    nat->discovery_cfg.NfcIP_Tgt_Mode_Config = nat->p2p_target_modes; //target
    ...
    /* Register callback for Card Emulation From Host Notification */
#ifdef HOST_EMULATION
    TRACE("Register callback for Card Emulation From Host Notification");
    REENTRANCE_LOCK();
    ret = phLibNfc_CardEmulation_NtfRegister(nfc_jni_cefh_notification_callback,
            (void *) nat);
    REENTRANCE_UNLOCK();
    if (ret != NFCSTATUS_SUCCESS)
    {
        ALOGD("phLibNfc_CardEmulation_NtfRegister returned 0x%02x", ret);
        goto clean_and_return;
    }
    TRACE("phLibNfc_CardEmulation_NtfRegister returned  0x%x\n", ret);
  #endif
    /* ====== END ======= */
    ...
    clean_and_return: if (result != TRUE)
    {
        if (nat)
        {
            kill_client(nat);
...
static void com_android_nfc_NfcManager_getUICCSessionData(uint8_t* out)
{
    ...
    TRACE("com_android_nfc_NfcManager_getUICCSessionData");
    ...
    for (iCount = 0; iCount < 8; iCount++)
    {
        ...
        if (!nfc_cb_data_init(&cb_data, NULL))
        {
            TRACE("Create the local semaphore FAILED!!!");
            break;
        }

        /* Test if External RF field is detected */
        InParam.buffer = uiccSessionAddrs[iCount];
        InParam.length = 3;
        ...
        REENTRANCE_LOCK();
        ret = phLibNfc_Mgt_IoCtl(gHWRef, NFC_MEM_READ, &InParam, &OutParam,
                nfc_jni_ioctl_callback, (void *) &cb_data);
        REENTRANCE_UNLOCK();
        ...
        out[iCount] = (uint8_t)(Output_Buff);
        Output_Buff = 0x00;
    }
}
...
void nfc_jni_reset_timeout_values()
{
    REENTRANCE_LOCK();
#ifdef REMOVE_THIS
    IsoXchgTimeout = NXP_ISO_XCHG_TIMEOUT;
    phLibNfc_SetHciTimeout(NXP_NFC_HCI_TIMEOUT);
    FelicaTimeout = NXP_FELICA_XCHG_TIMEOUT;
    MifareRawTimeout = NXP_MIFARE_XCHG_TIMEOUT;
#else
    IsoXchgTimeout = 0;
    phLibNfc_SetHciTimeout (NXP_NFC_HCI_TIMEOUT);
    FelicaTimeout = 0;
 ...
 */
static phNfc_sData_t get_target_uid(
        phLibNfc_sRemoteDevInformation_t *psRemoteDevInfo)
{
    ...
    switch (psRemoteDevInfo->RemDevType)
    {
        ...
        case phNfc_eISO14443_4B_PICC:
            uid.buffer =
                    psRemoteDevInfo->RemoteDevInfo.Iso14443B_Info.AtqB.AtqResInfo.Pupi;
            uid.length =
                    sizeof(psRemoteDevInfo->RemoteDevInfo.Iso14443B_Info.AtqB.AtqResInfo.Pupi);
            break;
...
/* P2P - LLCP callbacks */
static void nfc_jni_llcp_linkStatus_callback(
        void *pContext, phFriNfc_LlcpMac_eLinkStatus_t eLinkStatus)
{
    ...
    struct nfc_jni_native_data *nat =
            (nfc_jni_native_data *) pContextData->pContext;
    ...
    if (eLinkStatus == phFriNfc_LlcpMac_eLinkActivated)
    {
        ...
        {
            ALOGI(
                    "LLCP Link activated (LTO=%d, MIU=%d, OPTION=0x%02x, WKS=0x%02x)",
                    sLinkParams.lto, sLinkParams.miu, sLinkParams.option,
                    sLinkParams.wks);
    ...
    } else if (eLinkStatus == phFriNfc_LlcpMac_eLinkDeactivated)
    {
        ...
        e->CallVoidMethod(nat->manager,
                cached_NfcManager_notifyLlcpLinkDeactivated, nat->tag);
        if (e->ExceptionCheck())
        {
...
static void nfc_jni_checkLlcp_callback(void *context, NFCSTATUS status)
{
    struct nfc_jni_callback_data * pContextData =
            (struct nfc_jni_callback_data*) context;
...
static void nfc_jni_llcp_transport_listen_socket_callback(
        void *pContext, phLibNfc_Handle hIncomingSocket)
{
    ...
    TRACE(
            "nfc_jni_llcp_transport_listen_socket_callback socket handle = %p", (void*)hIncomingSocket);

    pthread_mutex_lock(&pMonitor->incoming_socket_mutex);

    /* Store the connection request */
    pListenData = (nfc_jni_listen_data_t*) malloc(
            sizeof(nfc_jni_listen_data_t));
    if (pListenData == NULL)
    {
        ALOGE(
                "Failed to create structure to handle incoming LLCP connection request");
        goto clean_and_return;
    }
    ...
    clean_and_return: pthread_mutex_unlock(&pMonitor->incoming_socket_mutex);
}
        ...
        uint8_t uNofRemoteDev, NFCSTATUS status)
{
    ...
    {
        LOG_CALLBACK("nfc_jni_cefh_notification_callback: Target deselected",
                status);

        /* Notify manager that a target was deselected */
        e->CallVoidMethod(nat->manager,
                cached_NfcManager_notifyCEFHDeActivated);
        if (e->ExceptionCheck())
        {
            ALOGE("Exception occured");
            kill_client(nat);
    ...
    {
        LOG_CALLBACK("nfc_jni_cefh_notification_callback", status);
        TRACE("Discovered %d tags", uNofRemoteDev);

        /* Reset device connected flag */
        device_connected_flag = 1;

        if ((psRemoteDevList->psRemoteDevInfo->RemDevType
                == phNfc_eISO14443_A_PCD)
                || (psRemoteDevList->psRemoteDevInfo->RemDevType
                        == phNfc_eISO14443_B_PCD))
        {
            TRACE("Reader for CE From Host Discovered \n");

            //tag_cls = e->GetObjectClass(nat->cached_CEFromHost);
            //if(e->ExceptionCheck())
            //{
            ...
            //e->SetIntField(tag, f,psRemoteDevList[target_index].hTargetDev);
            //TRACE("Remote Device handle = 0x%08x",psRemoteDevList[target_index].hTargetDev);
        }

        //TRACE("Store Handle\n");
        //CEFromHostHandle = psRemoteDevList[target_index].hTargetDev;
        set_CEFromHost_mHandle(
                (phLibNfc_Handle)(psRemoteDevList[target_index].hTargetDev));
        //if (nat->tag != NULL) {
        ...
        if ((psRemoteDevList->psRemoteDevInfo->RemDevType
                ...
                || (psRemoteDevList->psRemoteDevInfo->RemDevType
                        == phNfc_eISO14443_B_PCD))
        {
            /* Notify manager that the reader for Card Emulation From Host was found */
            e->CallVoidMethod(nat->manager,
                    cached_NfcManager_notifyCEFHActivated);
            if (e->ExceptionCheck())
            {
                ALOGE("Exception occured");
                kill_client(nat);
                                     ...
                                     uint8_t uNoOfRemoteDev)
{
    ...
    {
        if ((psRemoteDevList[i].psRemoteDevInfo->RemDevType
        ...
        uint8_t uNofRemoteDev, NFCSTATUS status)
{
    ...
    {
        LOG_CALLBACK(
                "nfc_jni_Discovery_notification_callback: Target deselected",
                status);

        /* Notify manager that a target was deselected */
        e->CallVoidMethod(nat->manager,
                cached_NfcManager_notifyTargetDeselected);
        if (e->ExceptionCheck())
        {
            ALOGE("Exception occured");
            kill_client(nat);
    ...
    } else
    {
        ...
        /* Reset device connected flag */
        device_connected_flag = 1;
        phLibNfc_sRemoteDevInformation_t *remDevInfo =
                psRemoteDevList[target_index].psRemoteDevInfo;
...
//#ifdef NXP_NFC_P2P_PRIO_SW
        switch (psRemoteDevList->psRemoteDevInfo->RemDevType)
        {
            ...
            case phNfc_eISO14443_4A_PICC:
            {
                typeA_flag = !typeA_flag;
                if (TRUE != typeA_flag)
                {
                    nfc_jni_restart_discovery_unlocked (nfc_jni_get_nat_ext(
                            NULL));return;
                }
                break;
            }
...
//#endif
                ...
                || (remDevInfo->RemDevType == phNfc_eNfcIP1_Target))
        {
            ...
            if (remDevInfo->RemDevType == phNfc_eNfcIP1_Initiator)
            {
                ...
                for (i = 0;
                        i < remDevInfo->RemoteDevInfo.NfcIP_Info.ATRInfo_Length;
                        i++)
                {
                    ALOGD("%02x ",
                            remDevInfo->RemoteDevInfo.NfcIP_Info.ATRInfo[i]);
                }

                generalBytes = e->NewByteArray(
        ...
        } else
        {
            ...
            if (data.length > 0)
            {
                e->SetByteArrayRegion(tagUid, 0, data.length,
                        (jbyte *) data.buffer);
            }
            ...
            nfc_jni_get_technology_tree(e, psRemoteDevList,
                    multi_protocol ? uNofRemoteDev : 1, &techList, &handleList,
                    &typeList);
        ...
        //Dejan e->DeleteLocalRef(tag);

        //Dejan e->DeleteLocalRef(generalBytes);
        //Dejan e->DeleteLocalRef(tagUid);
    }
}
                                         ...
                                         NFCSTATUS status)
{
    ...
    if (status == NFCSTATUS_SUCCESS)
    {
        switch (evt_type)
        {
                ...
                if (evt_info->UiccEvtInfo.aid.length <= AID_MAXLEN)
                {
                    ...
                    if (aid != NULL)
                    {
                        if (TRACE_ENABLED == 1)
                        {
                            ...
                            aid_str[0] = '\0';
                            for (i = 0;
                                    i < (int) (aid->length) && i < AID_MAXLEN;
                                    i++)
                            {
                                snprintf(&aid_str[i * 2], 3, "%02x",
                                        aid->buffer[i]);
                            }
                            ALOGD("> AID: %s", aid_str);
                        }
                        ...
                        e->SetByteArrayRegion((jbyteArray) tmp_array, 0,
                                aid->length, (jbyte *) aid->buffer);
                        if (e->ExceptionCheck())
                        {
                            goto error;
                        }
                    ...
                    e->SetByteArrayRegion((jbyteArray) data_array, 0,
                            data->length, (jbyte *) data->buffer);
                    if (e->ExceptionCheck())
                    {
                        goto error;
                    }
            ...
            case phLibNfc_eSE_EvtConnectivity:
            {
                ...
                TRACE("Notify Nfc Service");
                e->CallVoidMethod(nat->manager,
                        cached_NfcManager_notifyConnectivityListeners);
            }
                break;
            ...
            case phLibNfc_eSE_EvtApduReceived:
            {
                ...
                TRACE("Notify Nfc Service");
                e->CallVoidMethod(nat->manager,
                        cached_NfcManager_notifySeApduReceived, tmp_array);
            }
                break;
            ...
            case phLibNfc_eSE_EvtCardRemoval:
            {
                ...
                TRACE("Notify Nfc Service");
                e->CallVoidMethod(nat->manager,
                        cached_NfcManager_notifySeEmvCardRemoval);
            }
                break;
            ...
            case phLibNfc_eSE_EvtMifareAccess:
            {
                ...
                e->SetByteArrayRegion((jbyteArray) tmp_array, 0, 2,
                        (jbyte *) mifare_command->buffer);
                if (e->ExceptionCheck())
                {
                    goto error;
                }
                TRACE("Notify Nfc Service");
                e->CallVoidMethod(nat->manager,
                        cached_NfcManager_notifySeMifareAccess, mifare_block);
            }
                break;
            ...
            case phLibNfc_eSE_EvtFieldOn:
            {
                ...
                TRACE("Notify Nfc Service");
                e->CallVoidMethod(nat->manager,
                        cached_NfcManager_notifySeFieldActivated);
            }
                break;
            ...
            case phLibNfc_eSE_EvtFieldOff:
            {
                ...
                TRACE("Notify Nfc Service");
                e->CallVoidMethod(nat->manager,
                        cached_NfcManager_notifySeFieldDeactivated);
            }
                break;
    ...
    clean_and_return: if (tmp_array != NULL)
    {
        e->DeleteLocalRef(tmp_array);
    }
}

static void nfc_jni_se_set_mode_callback(void *pContext, phLibNfc_Handle handle,
                                         NFCSTATUS status)
{
    struct nfc_jni_callback_data * pContextData =
            (struct nfc_jni_callback_data*) pContext;
                                           ...
                                           bool resume)
{
    ...
    REENTRANCE_LOCK();
    ret = phLibNfc_Mgt_ConfigureDiscovery(
            resume ? NFC_DISCOVERY_RESUME : NFC_DISCOVERY_CONFIG,
            nat->discovery_cfg, nfc_jni_discover_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();
    TRACE(
            "phLibNfc_Mgt_ConfigureDiscovery(%s-%s-%s-%s-%s-%s, %s-%x-%x) returned 0x%08x\n", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso14443A==TRUE?"3A":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso14443B==TRUE?"3B":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableFelica212==TRUE?"F2":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableFelica424==TRUE?"F4":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableNfcActive==TRUE?"NFC":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso15693==TRUE?"RFID":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.DisableCardEmulation==FALSE?"CE":"", nat->discovery_cfg.NfcIP_Mode, nat->discovery_cfg.Duration, ret);
...
static void nfc_jni_restart_discovery_unlocked(struct nfc_jni_native_data *nat)
{
    ...
    TRACE("nfc_jni_restart_discovery_unlocked - start");
    ...
    TRACE("******  Start NFC Discovery ******");
    REENTRANCE_LOCK();
    ret = phLibNfc_Mgt_ConfigureDiscovery(NFC_DISCOVERY_RESUME,
            nat->discovery_cfg, nfc_jni_discover_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();
    TRACE(
            "phLibNfc_Mgt_ConfigureDiscovery(%s-%s-%s-%s-%s-%s, %s-%x-%x) returned 0x%08x\n", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso14443A==TRUE?"3A":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso14443B==TRUE?"3B":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableFelica212==TRUE?"F2":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableFelica424==TRUE?"F4":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableNfcActive==TRUE?"NFC":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso15693==TRUE?"RFID":"", nat->discovery_cfg.PollDevInfo.PollCfgInfo.DisableCardEmulation==FALSE?"CE":"", nat->discovery_cfg.NfcIP_Mode, nat->discovery_cfg.Duration, ret);
    ...
    TRACE(
            "nfc_jni_restart_discovery_unlocked - Restarting polling loop- without Waiting for callback response");

    /* Not Waiting for callback response */

    TRACE("nfc_jni_restart_discovery_unlocked - end");
    clean_and_return: nfc_cb_data_deinit(&cb_data);
...
static void nfc_jni_stop_discovery_locked(struct nfc_jni_native_data *nat)
{
    ...
    memset((void *) &discovery_cfg, 0x00, sizeof(discovery_cfg));
    discovery_cfg.PollDevInfo.PollEnabled = 0;
    discovery_cfg.Duration = 300000; /* in ms */
    discovery_cfg.NfcIP_Mode = phNfc_eDefaultP2PMode;
    discovery_cfg.NfcIP_Tgt_Mode_Config = 0;
    discovery_cfg.NfcIP_Tgt_Disable = TRUE;

    /* Start Polling loop */
    TRACE("******  Stop NFC Discovery ******");
    REENTRANCE_LOCK();
    ret = phLibNfc_Mgt_ConfigureDiscovery(NFC_DISCOVERY_CONFIG, discovery_cfg,
            nfc_jni_discover_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();
    TRACE(
            "phLibNfc_Mgt_ConfigureDiscovery(%s-%s-%s-%s-%s-%s, %s-%x-%x) returned 0x%08x\n", discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso14443A==TRUE?"3A":"", discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso14443B==TRUE?"3B":"", discovery_cfg.PollDevInfo.PollCfgInfo.EnableFelica212==TRUE?"F2":"", discovery_cfg.PollDevInfo.PollCfgInfo.EnableFelica424==TRUE?"F4":"", discovery_cfg.PollDevInfo.PollCfgInfo.EnableNfcActive==TRUE?"NFC":"", discovery_cfg.PollDevInfo.PollCfgInfo.EnableIso15693==TRUE?"RFID":"", discovery_cfg.PollDevInfo.PollCfgInfo.DisableCardEmulation==FALSE?"CE":"", discovery_cfg.NfcIP_Mode, discovery_cfg.Duration, ret);
...
static void com_android_nfc_NfcManager_enableDiscovery(JNIEnv *e, jobject o)
{
     ...
     * notification callback*/REENTRANCE_LOCK();
    ret = phLibNfc_RemoteDev_NtfRegister(&nat->registry_info,
            nfc_jni_Discovery_notification_callback, (void *) nat);
    REENTRANCE_UNLOCK();
    if (ret != NFCSTATUS_SUCCESS)
    {
    ...
    TRACE(
            "phLibNfc_RemoteDev_NtfRegister(%s-%s-%s-%s-%s-%s-%s-%s) returned 0x%x\n", nat->registry_info.Jewel==TRUE?"J":"", nat->registry_info.MifareUL==TRUE?"UL":"", nat->registry_info.MifareStd==TRUE?"Mi":"", nat->registry_info.Felica==TRUE?"F":"", nat->registry_info.ISO14443_4A==TRUE?"4A":"", nat->registry_info.ISO14443_4B==TRUE?"4B":"", nat->registry_info.NFC==TRUE?"P2P":"", nat->registry_info.ISO15693==TRUE?"R":"", ret);
...
#ifdef REMOVE_THIS
uint16_t getFelicaTimeout()
{
    return FelicaTimeout;
}
#else
uint32_t getFelicaTimeout()
{
    return FelicaTimeout;
}
#endif
...
#ifdef REMOVE_THIS //REMOVE_THIS - start
...
static void setIsoDepTimeout(jint timeout)
{
    if (timeout <= 4900)
    {
        ...
        IsoXchgTimeout = value;
#else
        IsoXchgTimeout = timeout;
#endif
...
#ifdef REMOVE_THIS
uint16_t getIsoXchgTimeout()
{
    return IsoXchgTimeout;
}
#else
...
static void setNfcATimeout(jint timeout)
{
    ...
    } else
    {
        ...
        MifareRawTimeout = 0x00;
        phLibNfc_SetHciTimeout(timeout);
    }
}

#ifdef REMOVE_THIS
uint16_t getMifareRawTimeout()
{
    return MifareRawTimeout;
}
#else
                                                    ...
                                                    jint tech)
{
    ...
    switch (tech)
    {
        ...
        case TARGET_TYPE_ISO14443_4:
#ifdef REMOVE_THIS
            timeout = IsoXchgTimeout & 0x0F; // lower 4 bits only
#else
            timeout = IsoXchgTimeout; // all bits
#endif
            if (timeout == 0)
            {
                timeout = phLibNfc_GetHciTimeout();
...
static jboolean com_android_nfc_NfcManager_init_native_struc(JNIEnv *e,
                                                             jobject o)
{
    NFCSTATUS status;
    ...
    cached_NfcManager_notifyCEFHActivated = e->GetMethodID(cls,
            "notifyCEFromHostActivated", "()V");

    cached_NfcManager_notifyCEFHDeActivated = e->GetMethodID(cls,
    ...
    if (nfc_jni_cache_object(e, "com/android/nfc/dhimpl/NativeNfcTag",
            &(nat->cached_NfcTag)) == -1)
    {
    ...
    if (nfc_jni_cache_object(e, "com/android/nfc/dhimpl/NativeP2pDevice",
...
static jboolean com_android_nfc_NfcManager_deinitialize(JNIEnv *e, jobject o)
{
    ...
    if (nfc_cb_data_init(&cb_data, NULL))
    {
        ...
        REENTRANCE_LOCK();
        status = phLibNfc_Mgt_DeInitialize(gHWRef, nfc_jni_deinit_callback,
                (void *) &cb_data);
        REENTRANCE_UNLOCK();
        if (status == NFCSTATUS_PENDING)
        {
            TRACE(
                    "phLibNfc_Mgt_DeInitialize() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
...
static jintArray com_android_nfc_NfcManager_doGetSecureElementList(JNIEnv *e,
                                                                   jobject o)
{
    NFCSTATUS ret;
    jintArray list = NULL;
...
#ifdef SE_CACHE_NOT_ENABLED
    phLibNfc_SE_List_t se_list[PHLIBNFC_MAXNO_OF_SE];
#else
    phLibNfc_SE_List_t *se_list = NULL;

    /* Retrieve native structure address */
    nat = nfc_jni_get_nat(e, o);

    if (NULL == nat | NULL == nat->SE_List)
    {
        goto clean_and_return;
    }
    ...
    for (i = 0; i < se_count; i++)
    {
        if (se_list[i].eSE_Type == phLibNfc_SE_Type_SmartMX)
        {
            e->SetIntArrayRegion(list, i, 1, (jint*) &se_list[i].eSE_Type);
...
//        e->SetIntArrayRegion(list, i, 1, (jint*)&se_list[i].hSecureElement);
    ...
    clean_and_return: return list;
    e->DeleteLocalRef(list);
}

static void com_android_nfc_NfcManager_doSelectSecureElement(JNIEnv *e,
                                                             jobject o,
                                                             jint seID)
{
    NFCSTATUS ret;
    ...
    for (i = 0; i < se_count; i++)
    {
        ...
        if (se_list[i].eSE_Type == seID)
        {
            seHandle = se_list[i].hSecureElement;
            ALOGD("Select SE ID #%d: 0x%04x", i, se_list[i].hSecureElement);
        }
    }
    TRACE("******  Select Secure Element ******");

    TRACE("phLibNfc_SE_SetMode(0x%08x, ...)", seID);
    /* Set SE mode - Virtual */REENTRANCE_LOCK();
    if (seID == phLibNfc_SE_Type_UICC)
    {
        TRACE("Select UICC");
        ret = phLibNfc_SE_SetMode(seHandle, phLibNfc_SE_ActModeVirtual,
                nfc_jni_se_set_mode_callback, (void *) &cb_data);
    } else
    {
        TRACE("Select SMX");
        ret = phLibNfc_SE_SetMode(seHandle, phLibNfc_SE_ActModeVirtualVolatile,
                nfc_jni_se_set_mode_callback, (void *) &cb_data);
    }
    REENTRANCE_UNLOCK();
    if (ret != NFCSTATUS_PENDING)
    {
        ALOGD("phLibNfc_SE_SetMode() returned 0x%04x[%s]", ret,
                nfc_jni_get_status_name(ret));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_SE_SetMode() returned 0x%04x[%s]", ret, nfc_jni_get_status_name(ret));
    ...
    if (seID == phLibNfc_SE_Type_UICC)
    {
        do
        {
            ...
            gInputParam.buffer = inp;
            gInputParam.length = 0x10;
            gOutputParam.buffer = resp;
            REENTRANCE_LOCK();
            ret = phLibNfc_Mgt_IoCtl(gHWRef, NFC_SWP_STATUS, &gInputParam,
                    &gOutputParam, nfc_jni_ioctl_callback, (void *) &cb_data);
            REENTRANCE_UNLOCK();
            if (ret != NFCSTATUS_PENDING)
            {
                ALOGE("phLibNfc_Mgt_IoCtl() returned 0x%04x[%s]", ret,
                        nfc_jni_get_status_name(ret));
                goto clean_and_return;
            }
            if (sem_wait(&cb_data.sem))
            ...
            if (resp[0] == 0x02 || resp[0] == 0x06)
            {
                ALOGD("UICC Select Successfull");
                break;
            } else if (resp[0] == 0x01)
            {
                ALOGD("UICC Select Waiting for SWP activation");
                continue;
            } else
            {
                ALOGE("UICC Select Failed!!!");
                break;
            }
        } while (1);
    }
...
static void com_android_nfc_NfcManager_doDeselectSecureElement(JNIEnv *e,
                                                               jobject o,
                                                               jint seID)
{
    NFCSTATUS ret;
    ...
    /* Set SE mode - Default */REENTRANCE_LOCK();
    if (seID == phLibNfc_SE_Type_UICC)
    {
        TRACE("Deselect UICC");
        ret = phLibNfc_SE_SetMode(seHandle, phLibNfc_SE_ActModeOff,
                nfc_jni_se_set_mode_callback, (void *) &cb_data);
    } else
    {
        TRACE("Deselect SMX");
        ret = phLibNfc_SE_SetMode(seHandle, phLibNfc_SE_ActModeDefault,
                nfc_jni_se_set_mode_callback, (void *) &cb_data);
    }
    REENTRANCE_UNLOCK();
    ...
    if (ret != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_SE_SetMode() returned 0x%04x[%s]", ret,
                nfc_jni_get_status_name(ret));
        goto clean_and_return;
    }
    TRACE(
    ...
    if (seID == phLibNfc_SE_Type_UICC)
    {
        do
        {
            ...
            if (resp[0] == 0x00 || resp[0] == 0x03 || resp[0] == 0x05)
            {
                ALOGD("UICC Deselect Successfull");
                break;
            } else
            {
                ALOGE("UICC Deselect Failed!!!");
                continue;
            }
        } while (1);
    }
...
static jboolean com_android_nfc_NfcManager_doCheckLlcp(JNIEnv *e, jobject o)
{
     ...
     */
    cb_data = (struct nfc_jni_callback_data*) malloc(
            sizeof(nfc_jni_callback_data));
    ...
    REENTRANCE_LOCK();
    ret = phLibNfc_Llcp_CheckLlcp(hLlcpHandle, nfc_jni_checkLlcp_callback,
            nfc_jni_llcp_linkStatus_callback, (void*) cb_data);
    REENTRANCE_UNLOCK();
    ...
    {
        ALOGE("phLibNfc_Llcp_CheckLlcp() returned 0x%04x[%s]", ret,
                nfc_jni_get_status_name(ret));
        freeData = true;
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_Llcp_CheckLlcp() returned 0x%04x[%s]", ret, nfc_jni_get_status_name(ret));
...
static jboolean com_android_nfc_NfcManager_doActivateLlcp(JNIEnv *e, jobject o)
{
    ...
    {
        TRACE(
                "phLibNfc_Llcp_Activate() returned 0x%04x[%s]", ret, nfc_jni_get_status_name(ret));
        return JNI_TRUE;
    } else
    {
        ALOGE("phLibNfc_Llcp_Activate() returned 0x%04x[%s]", ret,
                nfc_jni_get_status_name(ret));
        return JNI_FALSE;
    }
}

static jobject com_android_nfc_NfcManager_doCreateLlcpConnectionlessSocket(
        JNIEnv *e, jobject o, jint nSap, jstring sn)
{
    NFCSTATUS ret;
    ...
    /* Create socket */
    TRACE(
            "phLibNfc_Llcp_Socket(eType=phFriNfc_LlcpTransport_eConnectionLess, ...)");
    REENTRANCE_LOCK();
    ret = phLibNfc_Llcp_Socket(phFriNfc_LlcpTransport_eConnectionLess, NULL,
            &sWorkingBuffer, &hLlcpSocket,
            nfc_jni_llcp_transport_socket_err_callback, (void*) nat);
    REENTRANCE_UNLOCK();

    if (ret != NFCSTATUS_SUCCESS)
    {
        lastErrorStatus = ret;
        ALOGE("phLibNfc_Llcp_Socket() returned 0x%04x[%s]", ret,
                nfc_jni_get_status_name(ret));
        goto error;
    }
    TRACE(
            "phLibNfc_Llcp_Socket() returned 0x%04x[%s]", ret, nfc_jni_get_status_name(ret));
    ...
    {
        lastErrorStatus = ret;
        ALOGE("phLibNfc_Llcp_Bind() returned 0x%04x[%s]", ret,
                nfc_jni_get_status_name(ret));
        /* Close socket created */REENTRANCE_LOCK();
        ret = phLibNfc_Llcp_Close(hLlcpSocket);
        REENTRANCE_UNLOCK();
        goto error;
    }
    TRACE(
            "phLibNfc_Llcp_Bind() returned 0x%04x[%s]", ret, nfc_jni_get_status_name(ret));
    ...
    error: if (serviceName.buffer != NULL)
    {
...
static jobject com_android_nfc_NfcManager_doCreateLlcpServiceSocket(
        JNIEnv *e, jobject o, jint nSap, jstring sn, jint miu, jint rw,
        jint linearBufferLength)
{
    ...
    /* Create socket */
    TRACE(
            "phLibNfc_Llcp_Socket(hRemoteDevice=0x%08x, eType=phFriNfc_LlcpTransport_eConnectionOriented, ...)", hLlcpHandle);
    REENTRANCE_LOCK();
    ret = phLibNfc_Llcp_Socket(phFriNfc_LlcpTransport_eConnectionOriented,
            &sOptions, &sWorkingBuffer, &hLlcpSocket,
            nfc_jni_llcp_transport_socket_err_callback, (void*) nat);
    REENTRANCE_UNLOCK();
    ...
    ret = phLibNfc_Llcp_Listen(hLlcpSocket,
            nfc_jni_llcp_transport_listen_socket_callback, (void*) hLlcpSocket);
    REENTRANCE_UNLOCK();

    if (ret != NFCSTATUS_SUCCESS)
    {
        ALOGE("phLibNfc_Llcp_Listen() returned 0x%04x[%s]", ret,
                nfc_jni_get_status_name(ret));
        lastErrorStatus = ret;
        /* Close created socket */REENTRANCE_LOCK();
        ret = phLibNfc_Llcp_Close(hLlcpSocket);
        REENTRANCE_UNLOCK();
        goto error;
    }
    TRACE(
            "phLibNfc_Llcp_Listen() returned 0x%04x[%s]", ret, nfc_jni_get_status_name(ret));

    /* Create new NativeLlcpServiceSocket object */
    if (nfc_jni_cache_object(e,
            "com/android/nfc/dhimpl/NativeLlcpServiceSocket", &(serviceSocket))
            == -1)
    {
    ...
    f = e->GetFieldID(clsNativeLlcpServiceSocket, "mLocalLinearBufferLength",
            "I");
...
static jobject com_android_nfc_NfcManager_doCreateLlcpSocket(
        JNIEnv *e, jobject o, jint nSap, jint miu, jint rw,
        jint linearBufferLength)
{
    ...
    /* Create socket */
    TRACE(
            "phLibNfc_Llcp_Socket(eType=phFriNfc_LlcpTransport_eConnectionOriented, ...)");
    REENTRANCE_LOCK();
    ret = phLibNfc_Llcp_Socket(phFriNfc_LlcpTransport_eConnectionOriented,
            &sOptions, &sWorkingBuffer, &hLlcpSocket,
            nfc_jni_llcp_transport_socket_err_callback, (void*) nat);
    ...
    if (nfc_jni_cache_object(e, "com/android/nfc/dhimpl/NativeLlcpSocket",
            &(clientSocket)) == -1)
    {
        ALOGE("Llcp socket object creation error");
        return NULL;
    ...
    if (nSap != 0)
    {
        /* Bind socket */
        TRACE(
                "phLibNfc_Llcp_Bind(hSocket=0x%08x, nSap=0x%02x)", hLlcpSocket, nSap);
        REENTRANCE_LOCK();
...
static void com_android_nfc_NfcManager_doSetP2pInitiatorModes(JNIEnv *e,
                                                              jobject o,
                                                              jint modes)
{
...
static bool performDownload(struct nfc_jni_native_data* nat, bool takeLock)
{
    ...
    status = phLibNfc_Mgt_IoCtl(gHWRef, NFC_FW_DOWNLOAD, &gInputParam,
            &gOutputParam, nfc_jni_ioctl_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_Mgt_IoCtl() (download) returned 0x%04x[%s] %d", status,
                nfc_jni_get_status_name(status),__LINE__);
        result = FALSE;
        goto clean_and_return;
    }
    TRACE("phLibNfc_Mgt_IoCtl() (download) returned 0x%04x[%s]  %d", status, nfc_jni_get_status_name(status),__LINE__);
    ...
    /* NOTE: we will get NFCSTATUS_FEATURE_NOT_SUPPORTED when we
     ...
     error condition. */
    if (cb_data.status != NFCSTATUS_SUCCESS
            && cb_data.status != NFCSTATUS_FEATURE_NOT_SUPPORTED)
    {
        TRACE(
                "phLibNfc_Mgt_IoCtl() (download) returned 0x%04x[%s] %d", status, nfc_jni_get_status_name(status),__LINE__);
        result = FALSE;
        goto clean_and_return;
    }
    ...
    {
        ALOGW(
                "Old-style firmware not installed on top of new-style firmware. Using existing firmware in the chip.");
    }
...
static jboolean com_android_nfc_NfcManager_doSetMultiSEState(JNIEnv *e,
                                                             jobject o,
                                                             jboolean state)
{
...
//  ALOGE("com_android_nfc_NfcManager_doSetMultiSEState() - start");

    //FIXME: Allocating on stack. MAX size is 619 bytes.
    phHal_MultiEmulationCfg_t pMultiSeConfig;

    pMultiSeConfig.multiEmulationEnable = state;
    pMultiSeConfig.numAidEntries = 0;
    pMultiSeConfig.numProtocolEntries = 0;
    ...
    REENTRANCE_LOCK();
    ret = phLibNfc_Mgt_MultiEmulation_Config(&pMultiSeConfig,
            nfc_jni_set_multise_routing_table_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();
...
//  ALOGE("com_android_nfc_NfcManager_doSetMultiSEState() - result = %s",(JNI_TRUE==result)?"TRUE":"FALSE");
//  ALOGE("com_android_nfc_NfcManager_doSetMultiSEState() - end");
...
static jboolean com_android_nfc_NfcManager_doSetMultiSERoutingTable(
        JNIEnv *e, jobject o, jobjectArray routingInfos)
{

    jboolean result = JNI_FALSE;
    jboolean isDefaultRouteSet = JNI_FALSE;

//  ALOGE("com_android_nfc_NfcManager_doSeAidUpdate() - start");

    jint length;
    int ret;
    struct nfc_jni_callback_data cb_data;

#ifndef NFC_MULTISE_SUPPORTED
    ALOGI("MultiSE not supported!");
    return result;
#endif
    ...
    pMultiSeConfig.multiEmulationEnable = true;

    phHal_MultiEmulationAidCfg_t *aidList = pMultiSeConfig.aidList;
    phHal_MultiEmulationProtocolCfg_t *protocolInfoList =
            pMultiSeConfig.protocolInfo;
    ...
    if (length > 0
            && length
                    <= (NXP_NFC_MAX_AID_SUPPORT + NXP_NFC_MAX_PROTOCOL_SUPPORT))
    {
        ...
        jfieldID fRouteDetail = e->GetFieldID(routingInfo_cls, "mRouteDetail",
                "[I");
        jfieldID fLocation = e->GetFieldID(routingInfo_cls, "mLocation", "B");
        jfieldID fPowerState = e->GetFieldID(routingInfo_cls, "mPowerState",
                "B");
        ...
        for (jint iCount = 0; iCount < length; iCount++)
        {
            ...
            if (/*ROUTE_AID*/1 == routeType)
            {
                ...
                jintArray routeDetailIntArr = (jintArray) e->GetObjectField(
                        routingInfo, fRouteDetail);

                if (NULL != routeDetailIntArr)
                {
                    uint8_t *routeDetailBytes =
                            (uint8_t *) e->GetIntArrayElements(
...
//                  ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable()  aidBytes=%02x %02x , location=%02x , powerState=%02x , multiSeDefault=%d", routeDetailBytes[0],routeDetailBytes[1],location,powerState);

                    if (idxAid < NXP_NFC_MAX_AID_SUPPORT && len > 0
                            && len <= MAX_AID_LEN)
                    {
                        jint ibsel = 0;
                        jint offset = sizeof(jint) - 1;

                        for (ibsel = 0; ibsel < len; ibsel++)
                        {
                            aidList[idxAid].aid[ibsel] = routeDetailBytes[ibsel
                                    + offset * ibsel];
                        }

                        aidList[idxAid].aidLength = len;

                        aidList[idxAid].aidLocation =
                                (phNfc_eEmuLocation_t) location;
                        aidList[idxAid].powerState =
                        ...
                        /*
                         ...
                         for(i=0;i<len;i++)
                         {
                         ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable() aidinfo.aid[%d]=%02x",i,aidList[idxAid-1].aid[i]);
                         }

                         ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable()  aidBytesLen=%02d , location=%02x , powerState=%02x ", aidList[idxAid - 1].aidLength, aidList[idxAid -1].aidLocation, aidList[idxAid - 1].powerState);
                         */
                    }

                    /********Release elements*********/
                    e->ReleaseIntArrayElements(routeDetailIntArr,
                            (jint *) routeDetailBytes, JNI_ABORT);
...
//                  ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable()  ROUTE_PROTOCOL ==> routeDetailBytes=%02x  , location=%02x , powerState=%02x ", routeDetailBytes[0],location,powerState);

                    if (idxProtocol < NXP_NFC_MAX_PROTOCOL_SUPPORT && 1 == len)
                    {

                        protocolInfoList[idxProtocol].protocolType =
                                (phNfc_eEmuProtocol_t) routeDetailBytes[0];

                        protocolInfoList[idxProtocol].location =
                                (phNfc_eEmuLocation_t) location;
                        protocolInfoList[idxProtocol].powerState =
                                (phNfc_ePowerState_t) powerState;

                        idxProtocol++;

//                      ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable()  ROUTE_PROTOCOL ==> protocolType=%02d , location=%02x , powerState=%02x ", protocolInfoList[idxProtocol-1].protocolType, protocolInfoList[idxProtocol-1].location, protocolInfoList[idxProtocol-1].powerState);
...
//              ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable ROUTE_DEFAULT.");
                ...
                if (false == isDefaultRouteSet)
                {
                    uint8_t location = (uint8_t) e->GetByteField(routingInfo,
                            fLocation);
                    pMultiSeConfig.defaultRoute =
                            (phNfc_eEmuLocation_t) location;
            ...
            {
                ALOGE(
                        "com_android_nfc_NfcManager_doSetMultiSERoutingTable INVALID ROUTE_TYPE - IGNORING...");
            }

            pMultiSeConfig.numAidEntries = idxAid; //Since indx starts from zero(0)
            pMultiSeConfig.numProtocolEntries = idxProtocol; //Since indx starts from zero(0)

        }
//      ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable() pMultiSeConfig.numAidEntries=%02d",pMultiSeConfig.numAidEntries);
        e->DeleteLocalRef(routingInfo_cls);

//      ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable() - calling phLibNfc_Mgt_MultiEmulation_Config");
        ...
        {
            ALOGE(
                    "com_android_nfc_NfcManager_doSetMultiSERoutingTable() - ret != NFCSTATUS_PENDING hence returning false...");
            goto clean_and_return;
        }
...
//  ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable() - result = %s",(JNI_TRUE==result)?"TRUE":"FALSE");
//  ALOGE("com_android_nfc_NfcManager_doSetMultiSERoutingTable() - end");
...
static JNINativeMethod gMethods[] =
            {
                ...
                { "enableDiscovery", "()V",
                    ...
                    { "doCreateLlcpConnectionlessSocket",
                        "(ILjava/lang/String;)Lcom/android/nfc/dhimpl/NativeLlcpConnectionlessSocket;",
                    ...
                    { "doCreateLlcpSocket",
                        "(IIII)Lcom/android/nfc/dhimpl/NativeLlcpSocket;",
                ...
                /*MultiSE*/
                    { "doSetMultiSERoutingTable",
                        "([Landroid/nfc/MultiSERoutingInfo;)Z",
                        (void *) com_android_nfc_NfcManager_doSetMultiSERoutingTable },

                    { "doSetMultiSEState", "(Z)Z",
                        (void *) com_android_nfc_NfcManager_doSetMultiSEState },
            /*MultiSE*/
...
int register_com_android_nfc_NativeNfcManager(JNIEnv *e)
{
    ...
    return jniRegisterNativeMethods(e,
            "com/android/nfc/dhimpl/NativeNfcManager", gMethods,
            NELEM(gMethods));
}
