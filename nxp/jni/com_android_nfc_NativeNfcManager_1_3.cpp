...
static uint32_t timerWTXID=PH_OSALNFC_INVALID_TIMER_ID;
static uint32_t timerWtxValue;
...
#ifdef HOST_EMULATION
static void nfc_jni_cefh_notification_callback(void *pContext,
        phLibNfc_RemoteDevList_t *psRemoteDevList,
        uint8_t uNofRemoteDev, NFCSTATUS status);
#endif
...
static int nfc_jni_initialize(struct nfc_jni_native_data *nat) {
   ...
   ALOGD("Start Initialization libnfc 1.3\n");
...
force_download:
   ...
   TRACE("phLibNfc_Mgt_SetLlcp_ConfigParams returned 0x%04x[%s]", status,
    ...
    /* ===== DISCOVERY ==== */
    nat->discovery_cfg.NfcIP_Tgt_Disable = FALSE;//davis_JBMR2
    nat->discovery_cfg.NfcIP_Mode = nat->p2p_initiator_modes; //initiator
    nat->discovery_cfg.NfcIP_Tgt_Mode_Config = nat->p2p_target_modes; //target//davis_JBMR2
...
void jniWTXtimeout(uint32_t WtxTimerID, void *WtxTimerContext)
{
   nfc_jni_native_data *nat;
   nat=nfc_jni_get_nat_ext(NULL);
   ALOGE("jniWTXtimeout");
   emergency_recovery(nat);
}
void jniTimerCreateWTX()
{
    ALOGE("jniTimerCreateWTX");
    if( PH_OSALNFC_INVALID_TIMER_ID == timerWTXID)
    {
       timerWTXID = phOsalNfc_Timer_Create();
    }
}
void jniTimerStartWTX(uint32_t RegTimeCnt)
{
   if( PH_OSALNFC_INVALID_TIMER_ID != timerWTXID)
   {
       ALOGD("jniTimerStartWTX  %d", RegTimeCnt);
       timerWtxValue=RegTimeCnt;
       phOsalNfc_Timer_Start(timerWTXID,RegTimeCnt,jniWTXtimeout,0);
   }
}
void jniTimerRestartWTX(void)
{
    if( PH_OSALNFC_INVALID_TIMER_ID != timerWTXID)
    {
       ALOGE("jniTimerRestartWTX");
       phOsalNfc_Timer_Stop(timerWTXID );
       phOsalNfc_Timer_Start(timerWTXID,timerWtxValue,jniWTXtimeout,0);
    }
}
void jniTimerStopWTX(void )
{
    if( PH_OSALNFC_INVALID_TIMER_ID != timerWTXID)
    {
       ALOGE("jniTimerStopWTX");
       phOsalNfc_Timer_Stop(timerWTXID );
       timerWtxValue=0;
    }
}
void jniTimerDeleteWTX(void)
{
    ALOGE("jniTimerDeleteWTX");
    if( PH_OSALNFC_INVALID_TIMER_ID != timerWTXID)
    {
       phOsalNfc_Timer_Delete(timerWTXID);
       timerWTXID=PH_OSALNFC_INVALID_TIMER_ID;
    }
}
...
static phNfc_sData_t get_target_uid(phLibNfc_sRemoteDevInformation_t *psRemoteDevInfo)
{
    ...
    case phNfc_eKovio_PICC:
        uid.length=psRemoteDevInfo->RemoteDevInfo.kovio_Info.UidLength;
        uid.buffer = psRemoteDevInfo->RemoteDevInfo.kovio_Info.Uid;
        break;
#endif
        default:
            uid.buffer = NULL;
   ...
   phLibNfc_uSeEvtInfo_t *evt_info, NFCSTATUS status)
{
    ...
    if (status == NFCSTATUS_SUCCESS)
    {
        switch (evt_type)
        {
            ...
            case phLibNfc_eSE_EvtTypeWTX:
            {
              TRACE("Notify Nfc Service  phLibNfc_eSE_EvtTypeWTX");
              jniTimerRestartWTX();
            }
                break;
...
static void nfc_jni_start_discovery_locked(struct nfc_jni_native_data *nat, bool resume)
{
   ...
   nat->discovery_cfg.NfcIP_Tgt_Mode_Config = nat->p2p_target_modes; //davis_JBMR2
...
static void nfc_jni_stop_discovery_locked(struct nfc_jni_native_data *nat)
{
   ...
   int numRetries = 3;
    ...
    memset((void *) &discovery_cfg, 0x00, sizeof(discovery_cfg));//davis_JBMR2
    discovery_cfg.PollDevInfo.PollEnabled = 0;
    discovery_cfg.Duration = 300000; /* in ms *///davis_JBMR2
    discovery_cfg.NfcIP_Mode = phNfc_eDefaultP2PMode;
    discovery_cfg.NfcIP_Tgt_Mode_Config = 0;//davis_JBMR2
...
static void com_android_nfc_NfcManager_doSelectSecureElement(JNIEnv *e, jobject o) {
    ALOGW("%s :: Card emulation not supported", __FUNCTION__);
}
...
static jint com_android_nfc_NfcManager_doGetHWVersionInfo(JNIEnv *e, jobject o)
{
    jint result;
    NFCSTATUS status;
    struct nfc_jni_native_data *nat = NULL;
    ...
    nat = nfc_jni_get_nat(e,o);
    status  = phLibNfc_Mgt_GetstackCapabilities(&caps,(void*)nat);
    result = NXP_NFC_ROM_INFO(caps.psDevCapabilities.fw_version);
    return result;
}
#endif
/*
...
static JNINativeMethod gMethods[] =
{
    ...
    {"doGetHWVersionInfo", "()I",
         (void *)com_android_nfc_NfcManager_doGetHWVersionInfo},
#endif
};
