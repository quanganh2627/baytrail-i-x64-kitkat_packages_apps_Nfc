...
static void nfc_jni_connect_callback(
        void *pContext, phLibNfc_Handle hRemoteDev,
...
static void nfc_jni_checkndef_callback(void *pContext,
                                       phLibNfc_ChkNdef_Info_t info,
                                       NFCSTATUS status)
{
    struct nfc_jni_callback_data * pCallbackData =
            (struct nfc_jni_callback_data *) pContext;
    LOG_CALLBACK("nfc_jni_checkndef_callback", status);
    phLibNfc_ChkNdef_Info_t* pNdefInfo =
            (phLibNfc_ChkNdef_Info_t*) (pCallbackData->pContext);
...
static void nfc_jni_transceive_callback(void *pContext, phLibNfc_Handle handle,
...
static jbyteArray com_android_nfc_NativeNfcTag_doRead(JNIEnv *e, jobject o)
{
    ...
    REENTRANCE_LOCK();
    status = phLibNfc_Ndef_Read(handle, &nfc_jni_ndef_rw, phLibNfc_Ndef_EBegin,
            nfc_jni_tag_rw_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_Ndef_Read() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_Ndef_Read() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
...
static jboolean com_android_nfc_NativeNfcTag_doWrite(JNIEnv *e, jobject o,
                                                     jbyteArray buf)
{
    NFCSTATUS status;
    ...
    REENTRANCE_LOCK();
    status = phLibNfc_Ndef_Write(handle, &nfc_jni_ndef_rw,
            nfc_jni_tag_rw_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_Ndef_Write() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_Ndef_Write() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
    ...
    clean_and_return: e->ReleaseByteArrayElements(buf,
            (jbyte *) nfc_jni_ndef_rw.buffer, JNI_ABORT);
 ...
 */
static void set_target_pollBytes(
        JNIEnv *e, jobject tag,
        ...
        switch (techId[tech])
        {
            ...
            case TARGET_TYPE_ISO14443_3A:
                ...
                } else
                {
                    pollBytes =
                            e->NewByteArray(
                                    sizeof(psRemoteDevInfo->RemoteDevInfo.Iso14443A_Info.AtqA));
                    e->SetByteArrayRegion(pollBytes, 0,
                            sizeof(psRemoteDevInfo->RemoteDevInfo.Iso14443A_Info.AtqA),
            ...
            case TARGET_TYPE_ISO14443_3B:
                pollBytes =
                        e->NewByteArray(
                                sizeof(psRemoteDevInfo->RemoteDevInfo.Iso14443B_Info.AtqB.AtqResInfo.AppData)
            ...
            case TARGET_TYPE_FELICA:
                pollBytes =
                        e->NewByteArray(
                                sizeof(psRemoteDevInfo->RemoteDevInfo.Felica_Info.PMm)
            ...
            case TARGET_TYPE_ISO15693:
                pollBytes =
                        e->NewByteArray(
                                sizeof(psRemoteDevInfo->RemoteDevInfo.Iso15693_Info.Flags)
 ...
 */
static void set_target_activationBytes(
        JNIEnv *e, jobject tag,
        phLibNfc_sRemoteDevInformation_t *psRemoteDevInfo)
{
    jclass tag_cls = e->GetObjectClass(tag);
    ...
    for (int tech = 0; tech < techListLength; tech++)
    {
        switch (techId[tech])
        {
            ...
            case TARGET_TYPE_ISO14443_3A:
                actBytes =
                        e->NewByteArray(
                                sizeof(psRemoteDevInfo->RemoteDevInfo.Iso14443A_Info.Sak));
            ...
            case TARGET_TYPE_ISO14443_4:
                ...
                if (psRemoteDevInfo->RemDevType == phNfc_eISO14443_B_PICC
                        || psRemoteDevInfo->RemDevType
                                == phNfc_eISO14443_4B_PICC)
                {
                    actBytes =
                            e->NewByteArray(
                                    psRemoteDevInfo->RemoteDevInfo.Iso14443B_Info.HiLayerRespLength);
                    e->SetByteArrayRegion(actBytes, 0,
                            psRemoteDevInfo->RemoteDevInfo.Iso14443B_Info.HiLayerRespLength,
...
static jint com_android_nfc_NativeNfcTag_doConnect(JNIEnv *e, jobject o,
                                                   phLibNfc_Handle handle)
{
    ...
    REENTRANCE_LOCK();
    storedHandle = handle;
    status = phLibNfc_RemoteDev_Connect(handle, nfc_jni_connect_callback,
            (void *) &cb_data);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_RemoteDev_Connect(RW) returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_RemoteDev_Connect(RW) returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
...
static jint com_android_nfc_NativeNfcTag_doHandleReconnect(
        JNIEnv *e, jobject o, phLibNfc_Handle handle)
{
    ...
    REENTRANCE_LOCK();
    storedHandle = handle;
    status = phLibNfc_RemoteDev_ReConnect(handle, nfc_jni_connect_callback,
            (void *) &cb_data);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_RemoteDev_ReConnect(RW) returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_RemoteDev_ReConnect(RW) returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
...
static jint com_android_nfc_NativeNfcTag_doReconnect(JNIEnv *e, jobject o)
{
    ...
    if (libNfcType != -1)
    {
        ...
        if ((libNfcType != phNfc_eJewel_PICC)
                && (libNfcType != phNfc_eISO15693_PICC))
        {
            phLibNfc_Handle handle = nfc_jni_get_connected_handle(e, o);
...
static jboolean com_android_nfc_NativeNfcTag_doDisconnect(JNIEnv *e, jobject o)
{
    ...
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_RemoteDev_Disconnect(%x) returned 0x%04x[%s]", handle,
                status, nfc_jni_get_status_name(status));
        nfc_jni_restart_discovery_locked(nfc_jni_get_nat_ext(e));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_RemoteDev_Disconnect(%x) returned 0x%04x[%s]", handle, status, nfc_jni_get_status_name(status));
...
static uint16_t crc_16_ccitt1(uint8_t* msg, size_t len, uint16_t init)
{
    uint16_t b, crc = init;
...
static void nfc_insert_crc_a(uint8_t* msg, size_t len)
{
    uint16_t crc;
...
static void nfc_get_crc_a(uint8_t* msg, size_t len, uint8_t* byte1,
                          uint8_t* byte2)
{
    uint16_t crc;
...
static bool crc_valid(uint8_t* msg, size_t len)
{
    uint8_t crcByte1, crcByte2;

    nfc_get_crc_a(nfc_jni_transceive_buffer->buffer, len - 2, &crcByte1,
            &crcByte2);

    if (msg[len - 2] == crcByte1 && msg[len - 1] == crcByte2)
    {
        return true;
    } else
    {
...
static jbyteArray com_android_nfc_NativeNfcTag_doTransceive(
        JNIEnv *e, jobject o, jbyteArray data, jboolean raw,
        jintArray statusTargetLost)
{
    ...
    transceive_info.timeout = nfc_jni_get_timeout(e, o);
    ...
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_RemoteDev_Transceive() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
    ...
    TRACE(
            "phLibNfc_RemoteDev_Transceive() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
    ...
    {
        if (crc_valid(nfc_jni_transceive_buffer->buffer,
    ...
    } else
    {
        ...
        if (result != NULL)
        {
            e->SetByteArrayRegion(result, 0, nfc_jni_transceive_buffer->length,
                    (jbyte *) nfc_jni_transceive_buffer->buffer);
        }
    }
    clean_and_return: if (transceive_info.sRecvData.buffer != NULL)
    {
                                                     ...
                                                     jintArray ndefinfo)
{
    ...
    REENTRANCE_LOCK();
    status = phLibNfc_Ndef_CheckNdef(handle, nfc_jni_checkndef_callback,
            (void *) &cb_data);
    REENTRANCE_UNLOCK();
    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_Ndef_CheckNdef() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_Ndef_CheckNdef() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
    ...
    clean_and_return: e->ReleaseIntArrayElements(ndefinfo, ndef, 0);
    nfc_cb_data_deinit(&cb_data);
    CONCURRENCY_UNLOCK();
    return status;
}

static jboolean com_android_nfc_NativeNfcTag_doPresenceCheck(JNIEnv *e,
                                                             jobject o)
{
    phLibNfc_Handle handle = 0;
    NFCSTATUS status;
    ...
    REENTRANCE_LOCK();
    status = phLibNfc_RemoteDev_CheckPresence(handle,
            nfc_jni_presencecheck_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();

    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_RemoteDev_CheckPresence() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_RemoteDev_CheckPresence() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
...
static jboolean com_android_nfc_NativeNfcTag_doIsIsoDepNdefFormatable(
        JNIEnv *e, jobject o, jbyteArray pollBytes, jbyteArray actBytes)
{
            ...
            && actBytes != NULL && (e->GetArrayLength(actBytes) >= 1))
    {
        ...
        if (act[0] == 0x20 && poll[1] == 0x03)
        {
            ...
            jbyteArray respBytes = com_android_nfc_NativeNfcTag_doTransceive(e,
                    o, versionCmd, JNI_TRUE, NULL);
            if (respBytes != NULL)
            {
                ...
                e->ReleaseByteArrayElements(respBytes, (jbyte *) resp,
                        JNI_ABORT);
            }
        }
...
static jboolean com_android_nfc_NativeNfcTag_doNdefFormat(JNIEnv *e, jobject o,
                                                          jbyteArray key)
{
    phLibNfc_Handle handle = 0;
    NFCSTATUS status;
    ...
    REENTRANCE_LOCK();
    status = phLibNfc_RemoteDev_FormatNdef(handle, &keyBuffer,
            nfc_jni_formatndef_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();

    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("phLibNfc_RemoteDev_FormatNdef() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_RemoteDev_FormatNdef() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
    ...
    clean_and_return: e->ReleaseByteArrayElements(key,
            (jbyte *) keyBuffer.buffer, JNI_ABORT);
    nfc_cb_data_deinit(&cb_data);
    CONCURRENCY_UNLOCK();
    return result;
}

static jboolean com_android_nfc_NativeNfcTag_doMakeReadonly(JNIEnv *e,
                                                            jobject o,
                                                            jbyteArray key)
{
    phLibNfc_Handle handle = 0;
    ...
    REENTRANCE_LOCK();
    status = phLibNfc_ConvertToReadOnlyNdef_Extension(handle, &keyBuffer,
            nfc_jni_readonly_callback, (void *) &cb_data);
    REENTRANCE_UNLOCK();

    if (status != NFCSTATUS_PENDING)
    {
        ALOGE("pphLibNfc_ConvertToReadOnlyNdef() returned 0x%04x[%s]", status,
                nfc_jni_get_status_name(status));
        goto clean_and_return;
    }
    TRACE(
            "phLibNfc_ConvertToReadOnlyNdef() returned 0x%04x[%s]", status, nfc_jni_get_status_name(status));
...
{
    return jniRegisterNativeMethods(e, "com/android/nfc/dhimpl/NativeNfcTag",
