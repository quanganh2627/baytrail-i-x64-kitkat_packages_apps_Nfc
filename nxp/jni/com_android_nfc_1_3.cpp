...
uint32_t nfc_jni_get_timeout(JNIEnv *e, jobject o){
     int tech = 0;
     uint16_t timeout = 0;
    tech = nfc_jni_get_connected_technology(e, o);
   switch (tech) {
        ...
        case TARGET_TYPE_ISO14443_3A:
           timeout = getMifareRawTimeout();
            if (timeout == 0) {
            #if (MIFARE_TIMEOUT_CUSTOM > 0)
              timeout =MIFARE_TIMEOUT_CUSTOM;
            #else
        ...
        case TARGET_TYPE_ISO14443_4:
            timeout = getIsoXchgTimeout() & 0x0F; // lower 4 bits only
            if (timeout == 0) {
            #if ( MIFARE_DESFIRE_TIMEOUT_CUSTOM > 0)
                timeout =MIFARE_DESFIRE_TIMEOUT_CUSTOM;
             #else
                timeout = phLibNfc_GetHciTimeout();
             #endif
            } else {
        ...
        case TARGET_TYPE_FELICA:
            timeout = getFelicaTimeout();
            if (timeout == 0) {
             #if (FELICA_TIMEOUT_CUSTOM > 0)
               timeout =FELICA_TIMEOUT_CUSTOM;
             #else
                timeout = phLibNfc_GetHciTimeout();
             #endif
            } else {
...
jshort nfc_jni_get_connected_technology_sak(JNIEnv *e, jobject o)
{
   ...
   f = e->GetFieldID(c, "mTechExtras", "[Landroid/os/Bundle;");
   jobjectArray techExtrasObjectArray =  (jobjectArray) e->GetObjectField(o, f);




   if ((connectedTechIndex != -1) && (techExtrasObjectArray != NULL) &&
            (connectedTechIndex < e->GetArrayLength(techExtrasObjectArray))) {

      jobject bundle = e->GetObjectArrayElement(techExtrasObjectArray, connectedTechIndex);

        if (bundle != NULL) {
                  c = e->GetObjectClass(bundle);
                  m = e->GetMethodID(c, "getShort", "(Ljava/lang/String;)S");
                  jstring arg = e->NewStringUTF("sak");
                  sak = e->CallShortMethod(bundle,m,arg);
        }
    }
   ...
   for (int target = count - 1; target >= 0; target--) {
       ...
       switch (type)
       {
          ...
          case phNfc_eJewel_PICC:
           ...
           {
              index = addTechIfNeeded(technologies, handles, libnfctypes,
                      index, MAX_NUM_TECHNOLOGIES, TARGET_TYPE_KOVIO, handle, type);
            }break;
#endif
          default:
            {
